...
class ModelArgs:
    ...
    n_kv_heads: Optional[int] = (
        None  # Second type of heads for the KV Cache (grouped wuery attentions)
    )
    vocab_size: int = -1  # This will be set when we load the tokenizer
    multiple_of: int = 256
    ffn_dim_multiplier: Optional[float] = (
        None  # These last two parameters represent the hidden dimension of the ffn layer
    )
...
def repeat_kv(x: torch.Tensor, n_rep: int) -> torch.Tensor:
    ...
    return (
        ...
        )  # python automatically fills it if we use the expand function
        # (B, Seq_Len, N_KV_Heads * N_Rep, Head_Dim)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        batch_size, seq_len, _ = (
            x.shape
        )  # (B, 1, Dim) we know that the sequence length is 1
...
class FeedForward(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor
    ):  # the implementation starts from the definition of the forward method
...
class EncoderBlock(nn.Module):
    ...
    def __init__(self, args: ModelArgs):
        ...
        )  # here we call the RMSNorm block we defined previously
...
class Transformer(
    nn.Module
):  # This is the model repeated Nx times (except of the softmax)
    def __init__(self, args: ModelArgs) -> None:
        ...
        self.tok_embeddings = nn.Embedding(
            self.vocab_size, args.dim
        )  # convert inputs to embeddings

        self.layers = (
            nn.ModuleList()
        )  # list of the  layers that we pass the embeddings to
        for _ in range(args.n_layers):
            self.layers.append(
                EncoderBlock(args)
            )  # These are the block of which the model is composed of (norm, self attention, FF SwiGlu, etc)

        self.norm = RMSNorm(
            args.dim, eps=args.norm_eps
        )  # the last layer output is sent to the normalization
    ...
    def forward(self, tokens: torch.Tensor, start_pos: int):
        ...
        h = self.tok_embeddings(
            tokens
        )  # from tokens to embedding, note that Dim for this model is 4096

        # retrieve the pairs (m,theta) corresponding to the positions [ start_pos, start_pos + seq_len]
        freqs_complex = self.freqs_complex[
            start_pos : start_pos + seq_len
        ]  # we precompute something related to the positional encoding that we then pass ot the successive layer
